...
def scrape_page(browser, zipcode):
    ...
    desired_button = browser.find_element_by_xpath(
            '/html/body/div[1]/div[2]/div/div/div[2]/form/div[10]/div/div/div[4]/div/div[2]/span')          
    ...
    status_noresults = soup.find('tr',{
            'class':'ui-widget-content ui-datatable-empty-message'})
        ...
        if status == 'NONE_FOUND' or status == 'BAD_ZIP':
            ...
            for tr in status.findAll('tr')[1:]:
                ...
                for td in tr.findAll('td')[:4]:
                    if len(td.contents) > 1: # add data, if column info exists
                        rowList.append(td.contents[1])
            ...
            # After scraping every 200 zip codes, export the list to a csv file
            if count%200 == 0:
                print('Printing file: ' + code)
                # Convert storage container into pandas dataframe
                dropboxDF = pd.DataFrame(
                        dropboxList, columns = ['Business Name',
                                                'Address 1',
                                                'Address 2',
                                                'City, State Zip',
                                                'Map URL'])
                
                # Remove any duplicate entries caused by querying nearby
                #    zip codes
                dropboxDF.drop_duplicates(inplace=True)
                
                # Export dataframe to CSV file in the working directory
                filename = ('Data/October2018/DEA_addresses_October2018_' 
                            + str(code) + '.csv')
...
p.map(scrape_dea, zipBreakDown[0])
p.terminate()
p.join()